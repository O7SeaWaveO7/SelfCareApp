...
export type BadgeId =
  | 'first_step' | 'on_fire' | 'week_warrior' | 'month_master'
  | 'perfect_day' | 'collector' | 'century' | 'elite';

export const ALL_BADGES: { id: BadgeId; name: string; emoji: string; description: string }[] = [
  { id: 'first_step',   name: 'First Step',    emoji: '🌱', description: 'Complete your first habit' },
  { id: 'on_fire',      name: 'On Fire',        emoji: '🔥', description: 'Achieve a 3-day streak' },
  { id: 'week_warrior', name: 'Week Warrior',   emoji: '💪', description: 'Achieve a 7-day streak' },
  { id: 'month_master', name: 'Month Master',   emoji: '🌟', description: '30-day streak on any habit' },
  { id: 'perfect_day',  name: 'Perfect Day',    emoji: '⚡', description: 'Complete all habits in one day' },
  { id: 'collector',    name: 'Collector',      emoji: '🏆', description: 'Create 5 or more habits' },
  { id: 'century',      name: 'Century',        emoji: '💫', description: 'Earn 100 total points' },
...
export function getLevelInfo(points: number): LevelInfo {
  ...
  const progress = lv === LEVELS.length - 1 ? 1 : Math.min(1, (points - curr) / (next - curr));
  return { level: lv + 1, name: LEVELS[lv].name, progress, nextThreshold: next, currentThreshold: curr };
}
...
function computeEarnedBadges(habits: Habit[], totalPoints: number): BadgeId[] {
  ...
  if (habits.some(h => h.completedDates.length > 0))                     earned.push('first_step');
  if (habits.some(h => getCurrentStreak(h.completedDates) >= 3))          earned.push('on_fire');
  if (habits.some(h => getCurrentStreak(h.completedDates) >= 7))          earned.push('week_warrior');
  if (habits.some(h => getCurrentStreak(h.completedDates) >= 30))         earned.push('month_master');
  if (habits.length > 0 && habits.every(h => h.completedDates.includes(today))) earned.push('perfect_day');
...
type HabitsContextType = {
  ...
  editHabit: (id: string, name: string, emoji: string, color: string) => Promise<void>;
...
export function HabitsProvider({ children }: { children: React.ReactNode }) {
  ...
  const [stats,     setStats]     = useState<UserStats>({ totalPoints: 0, earnedBadges: [] });
  ...
  const editHabit = useCallback(async (id: string, name: string, emoji: string, color: string) => {
    await persist(habits.map(h => h.id === id ? { ...h, name, emoji, color } : h), stats);
  }, [habits, stats, persist]);

  const deleteHabit = useCallback(async (id: string) => {
    await persist(habits.filter(h => h.id !== id), stats);
  }, [habits, stats, persist]);

  const toggleHabit = useCallback(async (id: string): Promise<ToggleResult | null> => {
    const today  = todayStr();
    ...
    const newTotal     = isDone ? Math.max(0, stats.totalPoints - 10) : stats.totalPoints + pointsEarned;
    const allBadges    = computeEarnedBadges(newHabits, newTotal);
    const freshBadges  = allBadges.filter(b => !stats.earnedBadges.includes(b));
  ...
  return (
    <HabitsCtx.Provider value={{ habits, stats, isLoading, addHabit, editHabit, deleteHabit, toggleHabit }}>
      {children}
    </HabitsCtx.Provider>
  );
}
